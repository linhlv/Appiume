...
namespace Appiume.Apm.Configuration
{
    ...
    public class SettingManager : ISettingManager, ISingletonDependency
    {
        ...
        {
            return GetSettingValueInternalAsync(name, ApmSession.TenantId, ApmSession.UserId);
        }
        ...
        public async Task<IReadOnlyList<ISettingValue>> GetAllSettingValuesAsync(SettingScopes scopes)
        {
            ...
            if (scopes.HasFlag(SettingScopes.Application))
            {
                foreach (var settingValue in await GetAllSettingValuesForApplicationAsync())
                {
                    ...
                    if (!setting.IsInherited &&
                        ((setting.Scopes.HasFlag(SettingScopes.Tenant) && ApmSession.TenantId.HasValue) || (setting.Scopes.HasFlag(SettingScopes.User) && ApmSession.UserId.HasValue)))
                    {
                        continue;
                    }
            ...
            if (scopes.HasFlag(SettingScopes.Tenant) && ApmSession.TenantId.HasValue)
            {
                foreach (var settingValue in await GetAllSettingValuesForTenantAsync(ApmSession.TenantId.Value))
                {
                    var setting = settingDefinitions.GetOrDefault(settingValue.Name);
                    ...
                    if (!setting.IsInherited &&
                        (setting.Scopes.HasFlag(SettingScopes.User) && ApmSession.UserId.HasValue))
                    {
                        continue;
                    }
            ...
            if (scopes.HasFlag(SettingScopes.User) && ApmSession.UserId.HasValue)
            {
                foreach (var settingValue in await GetAllSettingValuesForUserAsync(ApmSession.ToUserIdentifier()))
                {
                    var setting = settingDefinitions.GetOrDefault(settingValue.Name);
        ...
        {
            return GetAllSettingValuesForUserAsync(new UserIdentifier(ApmSession.TenantId, userId));
        }
        ...
        {
            return ChangeSettingForUserAsync(new UserIdentifier(ApmSession.TenantId, userId), name, value);
        }
