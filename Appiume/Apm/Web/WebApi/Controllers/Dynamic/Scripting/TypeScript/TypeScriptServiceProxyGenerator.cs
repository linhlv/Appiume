...
namespace Appiume.Apm.Web.WebApi.Controllers.Dynamic.Scripting.TypeScript
{
    internal class TypeScriptServiceProxyGenerator : ITransientDependency
    {
        ...
        public string Generate(DynamicApiControllerInfo controllerInfo, string servicePrefix)
        {
            ...
            foreach (var methodInfo in _controllerInfo.Actions.Values)
            {
                ...
                if (returnType == "void")
                {
                    script.AppendLine(string.Format("           public {0} = function ({1}): apm.IPromise ",
                    ...
                    script.AppendLine("                        apm: true,");
                    script.AppendLine("                        url: apm.appPath + '" + ActionScriptingHelper.GenerateUrlWithParameters(_controllerInfo, methodInfo) + "',");
                ...
                {
                    script.AppendLine(string.Format("           public {0} = function ({1}): apm.IGenericPromise<{2}> ", methodInfo.ActionName.ToCamelCase(), 
            ...
            script.AppendLine("angular.module('apm').service('apm.services." + _controllerInfo.ServiceName.Replace("/", ".") + "', apm.services." + _controllerInfo.ServiceName.Replace("/", ".")+");");
