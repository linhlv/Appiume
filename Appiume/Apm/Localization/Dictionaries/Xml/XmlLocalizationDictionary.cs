...
using Appiume.Apm.Xml.Extensions;
...
namespace Appiume.Apm.Localization.Dictionaries.Xml
{
    ...
    public class XmlLocalizationDictionary : LocalizationDictionary
    {
        ...
        public static XmlLocalizationDictionary BuildFomFile(string filePath)
        {
            ...
            {
                throw new ApmException("Invalid localization file format! " + filePath, ex);
            }
        }
        ...
        public static XmlLocalizationDictionary BuildFomXmlString(string xmlString)
        {
            ...
            {
                throw new ApmException("A Localization Xml must include localizationDictionary as root node.");
            }

            var cultureName = localizationDictionaryNode[0].GetAttributeValueOrNull("culture");
            if (string.IsNullOrEmpty(cultureName))
            {
                throw new ApmException("culture is not defined in language XML file!");
            }
            ...
            if (textNodes != null)
            {
                foreach (XmlNode node in textNodes)
                {
                    ...
                    {
                        throw new ApmException("name attribute of a text is empty in given xml string.");
                    }
            ...
            {
                throw new ApmException("A dictionary can not contain same key twice. There are some duplicated names: " + dublicateNames.JoinAsString(", "));
            }
