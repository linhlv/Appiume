...
namespace Appiume.Apm.Ef
{
    ...
    /// </summary>
    public abstract class ApmDbContext : DbContext, ITransientDependency, IShouldInitialize
    {
        /// <summary>
        ...
        /// <summary>
        /// Constructor.
        /// Uses <see cref="IApmStartupConfiguration.DefaultNameOrConnectionString"/> as connection string.
        /// </summary>
        protected ApmDbContext()
        {
            InitializeDbContext();
        }
        ...
        /// Constructor.
        /// </summary>
        protected ApmDbContext(string nameOrConnectionString)
        ...
        /// Constructor.
        /// </summary>
        protected ApmDbContext(DbCompiledModel model)
            : base(model)
        {
            InitializeDbContext();
        }
        ...
        /// Constructor.
        /// </summary>
        protected ApmDbContext(DbConnection existingConnection, bool contextOwnsConnection)
        ...
        /// Constructor.
        /// </summary>
        protected ApmDbContext(string nameOrConnectionString, DbCompiledModel model)
        ...
        /// Constructor.
        /// </summary>
        protected ApmDbContext(ObjectContext objectContext, bool dbContextOwnsObjectContext)
        ...
        /// Constructor.
        /// </summary>
        protected ApmDbContext(DbConnection existingConnection, DbCompiledModel model, bool contextOwnsConnection)
        ...
        public virtual void Initialize()
        {
            ...
            this.SetFilterScopedParameterValue(ApmDataFilters.MustHaveTenant, ApmDataFilters.Parameters.TenantId, ApmSession.TenantId ?? 0);
            this.SetFilterScopedParameterValue(ApmDataFilters.MayHaveTenant, ApmDataFilters.Parameters.TenantId, ApmSession.TenantId);
        }

        protected override void OnModelCreating(DbModelBuilder modelBuilder)
        {
            base.OnModelCreating(modelBuilder);
            modelBuilder.Filter(ApmDataFilters.SoftDelete, (ISoftDelete d) => d.IsDeleted, false);
            modelBuilder.Filter(ApmDataFilters.MustHaveTenant, (IMustHaveTenant t, int tenantId) => t.TenantId == tenantId || (int?)t.TenantId == null, 0); //While "(int?)t.TenantId == null" seems wrong, it's needed. See https://github.com/jcachat/EntityFramework.DynamicFilters/issues/62#issuecomment-208198058
            modelBuilder.Filter(ApmDataFilters.MayHaveTenant, (IMayHaveTenant t, int? tenantId) => t.TenantId == tenantId, 0);
        }
        ...
        protected virtual void ApplyApmConcepts()
        {
        ...
        protected virtual void CheckAndSetMustHaveTenantIdProperty(object entityAsObj)
        {
            ...
            {
                throw new ApmException("Can not set TenantId to 0 for IMustHaveTenant entities!");
            }
        }
            ...
            if (userId.HasValue && entityAsObj is ICreationAudited)
            {
                ...
                if (entity.CreatorUserId == null)
                {
                    if (entity is IMayHaveTenant || entity is IMustHaveTenant)
                    {
                        ...
                        if ((entity is IMayHaveTenant && entity.As<IMayHaveTenant>().TenantId == ApmSession.TenantId) ||
                            (entity is IMustHaveTenant && entity.As<IMustHaveTenant>().TenantId == ApmSession.TenantId))
                        {
        ...
        protected virtual long? GetAuditUserId()
        {
            if (ApmSession.UserId.HasValue &&
                ...
                CurrentUnitOfWorkProvider.Current.GetTenantId() == ApmSession.TenantId)
            {
                return ApmSession.UserId;
            }
