...
using IPermissionManager = Appiume.Apm.Authorization.IPermissionManager;
using Permission = Appiume.Apm.Authorization.Permission;

namespace Appiume.Apm.Tenancy.Authorization.Users
{
    //TODO: Extract Login operations to ApmLoginManager and remove TTenant generic parameter.

    /// <summary>
    /// Extends <see cref="UserManager{TUser,TKey}"/> of ASP.NET Identity Framework.
    /// </summary>
    public abstract class ApmUserManager<TTenant, TRole, TUser>
        ...
        where TUser : ApmUser<TUser>
    {
        protected IUserPermissionStore<TUser> UserPermissionStore
        {
            get
            {
                ...
                {
                    throw new ApmException("Store is not IUserPermissionStore");
                }
        ...
        protected ApmRoleManager<TRole, TUser> RoleManager { get; private set; }

        protected ISettingManager SettingManager { get; private set; }

        protected ApmUserStore<TRole, TUser> ApmStore { get; private set; }
        ...
        protected ApmUserManager(
            ApmUserStore<TRole, TUser> userStore,
            ApmRoleManager<TRole, TUser> roleManager,
        ...
        public virtual async Task<bool> IsGrantedAsync(long userId, Permission permission)
        {
            ...
            if (!permission.MultiTenancySides.HasFlag(ApmSession.MultiTenancySide))
            {
                return false;
            }

            //Check for depended features
            if (permission.FeatureDependency != null && ApmSession.MultiTenancySide == MultiTenancySides.Tenant)
            {
        ...
        {
            return await ApmStore.FindByNameOrEmailAsync(userNameOrEmailAddress);
        }

        public virtual Task<List<TUser>> FindAllAsync(UserLoginInfo login)
        {
            return ApmStore.FindAllAsync(login);
        }

        [UnitOfWork]
        public virtual async Task<ApmLoginResult> LoginAsync(UserLoginInfo login, string tenancyName = null)
        {
        ...
        private async Task<ApmLoginResult> LoginAsyncInternal(UserLoginInfo login, string tenancyName)
        {
            ...
            else if (!string.IsNullOrWhiteSpace(tenancyName))
            {
                ...
                if (tenant == null)
                {
                    return new ApmLoginResult(ApmLoginResultType.InvalidTenancyName);
                }

                if (!tenant.IsActive)
                {
                    return new ApmLoginResult(ApmLoginResultType.TenantIsNotActive, tenant);
                }
            }
            ...
            {
                var user = await ApmStore.FindAsync(tenantId, login);
                if (user == null)
                {
                    return new ApmLoginResult(ApmLoginResultType.UnknownExternalLogin, tenant);
                }
        ...
        [UnitOfWork]
        public virtual async Task<ApmLoginResult> LoginAsync(string userNameOrEmailAddress, string plainPassword, string tenancyName = null)
        {
        ...
        private async Task<ApmLoginResult> LoginAsyncInternal(string userNameOrEmailAddress, string plainPassword, string tenancyName = null)
        {
            ...
            using (_unitOfWorkManager.Current.SetTenantId(tenantId))
            {
                ...
                var user = await ApmStore.FindByNameOrEmailAsync(tenantId, userNameOrEmailAddress);
                if (user == null)
                {
                    return new ApmLoginResult(ApmLoginResultType.InvalidUserNameOrEmailAddress, tenant);
                }
                ...
                if (!loggedInFromExternalSource)
                {
                    ...
                    {
                        return new ApmLoginResult(ApmLoginResultType.InvalidPassword, tenant, user);
                    }
                }
        ...
        private async Task<ApmLoginResult> CreateLoginResultAsync(TUser user, TTenant tenant = null)
        {
            if (!user.IsActive)
            {
                return new ApmLoginResult(ApmLoginResultType.UserIsNotActive);
            }

            if (await IsEmailConfirmationRequiredForLoginAsync(user.TenantId) && !user.IsEmailConfirmed)
            {
                return new ApmLoginResult(ApmLoginResultType.UserEmailIsNotConfirmed);
            }
            ...
            return new ApmLoginResult(tenant, user, await CreateIdentityAsync(user, DefaultAuthenticationTypes.ApplicationCookie));
        }

        private async Task SaveLoginAttempt(ApmLoginResult loginResult, string tenancyName, string userNameOrEmailAddress)
        {
            using (var uow = _unitOfWorkManager.Begin(TransactionScopeOption.Suppress))
            {
                ...
                using (_unitOfWorkManager.Current.SetTenantId(tenantId))
                {
                    ...
                    //TODO: We should replace this workaround with IClientInfoProvider when it's implemented in Apm (https://github.com/aspnetboilerplate/aspnetboilerplate/issues/926)
        ...
        /// <exception cref="ApmException">Throws exception if no user found with given id</exception>
        public virtual async Task<TUser> GetUserByIdAsync(long userId)
        {
            var user = await FindByIdAsync(userId);
            if (user == null)
            {
                throw new ApmException("There is no user with id: " + userId);
            }
        ...
        public async override Task<ClaimsIdentity> CreateIdentityAsync(TUser user, string authenticationType)
        {
            ...
            {
                identity.AddClaim(new Claim(ApmClaimTypes.TenantId, user.TenantId.Value.ToString(CultureInfo.InvariantCulture)));
            }
        ...
        public async override Task<IdentityResult> UpdateAsync(TUser user)
        {
            ...
            if (oldUserName == ApmUser<TUser>.AdminUserName && user.UserName != ApmUser<TUser>.AdminUserName)
            {
                return ApmIdentityResult.Failed(string.Format(L("CanNotRenameAdminUser"), ApmUser<TUser>.AdminUserName));
            }
        ...
        {
            if (user.UserName == ApmUser<TUser>.AdminUserName)
            {
                return ApmIdentityResult.Failed(string.Format(L("CanNotDeleteAdminUser"), ApmUser<TUser>.AdminUserName));
            }
        ...
        public virtual async Task<IdentityResult> ChangePasswordAsync(TUser user, string newPassword)
        {
            ...
            await ApmStore.SetPasswordHashAsync(user, PasswordHasher.HashPassword(newPassword));
        ...
        public virtual async Task<IdentityResult> CheckDuplicateUsernameOrEmailAddressAsync(long? expectedUserId, string userName, string emailAddress)
        {
            ...
            {
                return ApmIdentityResult.Failed(string.Format(L("Identity.DuplicateName"), userName));
            }

            user = (await FindByEmailAsync(emailAddress));
            if (user != null && user.Id != expectedUserId)
            {
                return ApmIdentityResult.Failed(string.Format(L("Identity.DuplicateEmail"), emailAddress));
            }
        ...
        private async Task CheckMaxUserOrganizationUnitMembershipCountAsync(int? tenantId, int requestedCount)
        {
            ...
            {
                throw new ApmException(string.Format("Can not set more than {0} organization unit for a user!", maxCount));
            }
        }
        ...
        public virtual Task<List<TUser>> GetUsersInOrganizationUnit(OrganizationUnit organizationUnit, bool includeChildren = false)
        {
            ...
            {
                var query = from uou in _userOrganizationUnitRepository.GetAll()
                            join user in ApmStore.Users on uou.UserId equals user.Id
        ...
        private async Task<bool> IsEmailConfirmationRequiredForLoginAsync(int? tenantId)
        {
            ...
            {
                return await SettingManager.GetSettingValueForTenantAsync<bool>(ApmTenancySettingNames.UserManagement.IsEmailConfirmationRequiredForLogin, tenantId.Value);
            }

            return await SettingManager.GetSettingValueForApplicationAsync<bool>(ApmTenancySettingNames.UserManagement.IsEmailConfirmationRequiredForLogin);
        }

        private async Task<TTenant> GetDefaultTenantAsync()
        {
            var tenant = await _tenantRepository.FirstOrDefaultAsync(t => t.TenancyName == ApmTenant<TUser>.DefaultTenantName);
            if (tenant == null)
            {
                throw new ApmException("There should be a 'Default' tenant if multi-tenancy is disabled!");
            }
        ...
        {
            return LocalizationManager.GetString(ApmTenancyConsts.LocalizationSourceName, name);
        }

        public class ApmLoginResult
        {
            public ApmLoginResultType Result { get; private set; }
            ...
            public ApmLoginResult(ApmLoginResultType result, TTenant tenant = null, TUser user = null)
            {
                Result = result;
                Tenant = tenant;
                User = user;
            ...
            public ApmLoginResult(TTenant tenant, TUser user, ClaimsIdentity identity)
                : this(ApmLoginResultType.Success, tenant)
            {
                User = user;
                Identity = identity;
            }
