...
namespace Appiume.Apm.Tenancy.Authorization.Roles
{
    ...
    /// </summary>
    public abstract class ApmRoleManager<TRole, TUser>
        ...
        where TUser : ApmUser<TUser>
    {
        ...
        private IRolePermissionStore<TRole, TUser> RolePermissionStore
        {
            get
            {
                ...
                {
                    throw new ApmException("Store is not IRolePermissionStore");
                }
        ...
        protected ApmRoleStore<TRole, TUser> ApmStore { get; private set; }
        ...
        /// Constructor.
        /// </summary>
        protected ApmRoleManager(
            ApmRoleStore<TRole, TUser> store,
        ...
        public async override Task<IdentityResult> DeleteAsync(TRole role)
        {
            if (role.IsStatic)
            {
                return ApmIdentityResult.Failed(string.Format(L("CanNotDeleteStaticRole"), role.Name));
            }
        ...
        /// <exception cref="ApmException">Throws exception if no role with given id</exception>
        public virtual async Task<TRole> GetRoleByIdAsync(int roleId)
        {
            var role = await FindByIdAsync(roleId);
            if (role == null)
            {
                throw new ApmException("There is no role with id: " + roleId);
            }
        ...
        /// <param name="roleName">Role name</param>
        /// <returns>Role</returns>
        /// <exception cref="ApmException">Throws exception if no role with given roleName</exception>
        public virtual async Task<TRole> GetRoleByNameAsync(string roleName)
        {
            var role = await FindByNameAsync(roleName);
            if (role == null)
            {
                throw new ApmException("There is no role with name: " + roleName);
            }
        ...
        public virtual async Task<IdentityResult> CheckDuplicateRoleNameAsync(int? expectedRoleId, string name, string displayName)
        {
            ...
            {
                return ApmIdentityResult.Failed(string.Format(L("RoleNameIsAlreadyTaken"), name));
            }

            role = await FindByDisplayNameAsync(displayName);
            if (role != null && role.Id != expectedRoleId)
            {
                return ApmIdentityResult.Failed(string.Format(L("RoleDisplayNameIsAlreadyTaken"), displayName));
            }
        ...
        {
            return ApmStore.FindByDisplayNameAsync(displayName);
        }
