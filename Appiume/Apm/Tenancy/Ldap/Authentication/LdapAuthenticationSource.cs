...
namespace Appiume.Apm.Tenancy.Ldap.Authentication
{
    /// <summary>
        ...
        where TUser : ApmUser<TUser>, new()
    {
        ...
        private readonly IApmTenancyLdapModuleConfig _ldapModuleConfig;

        protected LdapAuthenticationSource(ILdapSettings settings, IApmTenancyLdapModuleConfig ldapModuleConfig)
        {
            _settings = settings;
        ...
        public async override Task<TUser> CreateUserAsync(string userNameOrEmailAddress, TTenant tenant)
        {
            ...
            using (var principalContext = await CreatePrincipalContext(tenant))
            {
                ...
                {
                    throw new ApmException("Unknown LDAP user: " + userNameOrEmailAddress);
                }
        ...
        public async override Task UpdateUserAsync(TUser user, TTenant tenant)
        {
            ...
            using (var principalContext = await CreatePrincipalContext(tenant))
            {
                ...
                if (userPrincipal == null)
                {
                    throw new ApmException("Unknown LDAP user: " + user.UserName);
                }
        ...
        private async Task CheckIsEnabled(TTenant tenant)
        {
            ...
            {
                throw new ApmException("Ldap Authentication module is disabled globally!");                
            }

            var tenantId = GetIdOrNull(tenant);
            if (!await _settings.GetIsEnabled(tenantId))
            {
                throw new ApmException("Ldap Authentication is disabled for given tenant (id:" + tenantId + ")! You can enable it by setting '" + LdapSettingNames.IsEnabled + "' to true");
            }
        }
