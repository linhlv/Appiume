...
namespace Appiume.Apm.Tenancy.MultiTenancy
{
    /// <summary>
    /// Tenant manager.
    /// Implements domain logic for <see cref="ApmTenant{TTenant,TUser}"/>.
    /// </summary>
    ...
    public abstract class ApmTenantManager<TTenant, TRole, TUser> : IDomainService,
    ...
    {
        public ApmEditionManager EditionManager { get; set; }
        ...
        protected ApmTenantManager(
            IRepository<TTenant> tenantRepository, 
            IRepository<TenantFeatureSetting, long> tenantFeatureRepository,
            ApmEditionManager editionManager,
        ...
        public virtual async Task<IdentityResult> CreateAsync(TTenant tenant)
        {
            ...
            {
                return ApmIdentityResult.Failed(string.Format(L("TenancyNameIsAlreadyTaken"), tenant.TenancyName));
            }
        ...
        public virtual async Task<TTenant> GetByIdAsync(int id)
        {
            ...
            if (tenant == null)
            {
                throw new ApmException("There is no tenant with id: " + id);
            }
        ...
        {
            if (!Regex.IsMatch(tenancyName, ApmTenant<TUser>.TenancyNameRegex))
            {
                return ApmIdentityResult.Failed(L("InvalidTenancyName"));
            }
